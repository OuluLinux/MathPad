...
static string & add_print_prod(string & s, const symbolic & g, GIAC_CONTEXT) {
	...
	if (print_rewrite_prod_inv &&
		...
		if (d0.type < _CPLX || d0.type == _IDNT)
			...
			s += "(";
			add_print(s, d0, contextptr);
			s += ")"; // s += "("+d0.print(contextptr)+")";
		}
...
string & add_print(string & s, const gen & g, GIAC_CONTEXT) {
	...
	if (l && s[l-1] == '+') {
		...
		if (tmp.size() > 3 && (unsigned char) tmp[0] == 226 && (unsigned char) tmp[1] == 136 && (unsigned char) tmp[2] == 146) {
			// -30, -120, -110
			s[l-1] = '-';
		...
		try {
			...
			if (it != itend) {
				...
				case _SYMB: {
						...
						if (u == at_for) {
							if (state == nr_eval_for_cond) {
								// for in for_cond is for the syntax for x in l
								*destination = *it;
								++it;
								continue;
							}
...
#ifdef NSPIRE
template<class T> nio::ios_base<T> & operator << (nio::ios_base<T> & os, const symbolic & s) {
	return os << s.print(context0);
}

#else
ostream & operator << (ostream & os, const symbolic & s) {
	return os << s.print(context0);
}
