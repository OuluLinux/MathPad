...
static_help_t* get_static_help() {return (static_help_t*)static_help;}
...
void readhelp(vector<aide> & v, const char * f_name, int & count, bool warn) {
	count = 0;
#if !defined NSPIRE && !defined flagMSC
...
#if (!defined(NSPIRE_NEWLIB) || !defined(RTOS_THREADX)) && !defined(EMCC) && !defined(NSPIRE) && !defined flagMSC
...
#if ! (defined VISUALC || defined BESTA_OS || defined NSPIRE || defined NSPIRE_NEWLIB || defined NDLESS)
#ifdef WIN32
...
#if defined GNUWINCE || defined __MINGW_H || defined __ANDROID__ || defined EMCC || defined NSPIRE_NEWLIB || defined NDLESS || defined NDLESS || defined flagMSC
	return;
#else
