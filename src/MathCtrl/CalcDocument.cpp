...
void CalcDocument::GotAnswer(MathCore::Node n) {
	AddNode(n);
};
...
void CalcDocument::CursorLeft() {
	...
	MathCtrl::IteratorDeep it = cl.draw.BeginDeep();
	Node orig_ptr = cur.GetPtr();
	MathCtrl* begin = &cl.draw;
	MathCtrl* last = cur.GetMathCtrl();
	
	//PRINT("Before: " << cur.AsString());
	
	if (!orig_ptr)
		return;
	if (orig_ptr == begin.GetPtr() && !begin.IsRightHandFocus())
		return;
...
void CalcDocument::CursorRight() {
	...
	if (!orig_ptr || (orig_ptr == begin.GetPtr() && begin.IsRightHandFocus()))
		return;
